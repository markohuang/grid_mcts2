...
class NeutralAtomsEnv:
    ...
    def step(self, action: int, skip_obs: bool = False) -> StepResult:
        ...
        # Auto-execute layer when all relevant atoms have been placed
        reward = 0.0
        is_autoexec = self.current_atom_idx >= len(self.relevant_atoms)
        if is_autoexec:
        ...
        if self.reward_mode == 'plan_cost':
            # Option A delta, but suppress auto-execute reward (prevents telescoping)
            if is_autoexec:
                reward = 0.0
            else:
                cost_after = self._compute_remaining_cost()
                reward = (cost_before - cost_after) * self.reward_scale
        elif self.reward_mode == 'layer_delta':
