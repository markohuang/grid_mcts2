...
def main():
    ...
    def _main(_):
        ...
        print(f"train_offline: run_id={args.run_id} epoch={args.epoch} "
              f"training_steps={args.training_steps} preset={args.preset}")
        ...
        t0 = time.time()
        losses = trainer.fit(training_steps=args.training_steps)
        train_time = time.time() - t0
        if losses is None:
            raise RuntimeError("trainer.fit() returned None on a real network — "
                               "buffer check should have caught this.")
        print(f"  trained {args.training_steps} steps in {train_time:.1f}s "
